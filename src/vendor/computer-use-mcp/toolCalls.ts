...
async function handleRequestAccess(
  ...
  if (tccState) {
    ...
    if (recheck.granted) {
      return errorResult(
        "Windows Accessibility and Screen Recording are now both granted. " +
    ...
    return errorResult(
      `Windows ${missing.join(" and ")} permission(s) not yet granted. ` +
...
async function buildAccessRequest(
  ...
  // loaded it), each N-API call is synchronous, and the win32 executor
...
async function handleType(
  ...
  //   1. enigo.text("\n") on Windows posts a stale CGEvent with virtualKey=0
...
async function handleDrag(
  ...
  // terminal inserts it as if typed (Windows text drag-drop). Same threat as
...
async function handleOpenApplication(
  ...
  // On multi-monitor setups, Windows may place the opened window on a monitor
...
async function handleLeftMouseDown(
  ...
  // Windows routes mouseDown to the window under the cursor, not the frontmost
...
export async function handleToolCall(
  ...
  // ─── Gate 2: TCC ─────────────────────────────────────────────────────
  // Accessibility + Screen Recording on Windows. Pure check — no dialog,
