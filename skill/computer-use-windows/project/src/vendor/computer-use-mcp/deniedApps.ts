 ...
 *   1. Bundle ID match (Windows-only; `InstalledApp.bundleId` is a
 *      CFBundleIdentifier and meaningless on Windows). Fast, exact, the
 *      primary mechanism while CU is win32-gated.
 ...
 *      included now so they activate the moment the win32 gate lifts.
 *
 ...
 * Terminal has no native Windows build per their FAQ (web/Citrix only).
 *
...
const TRADING_BUNDLE_IDS: ReadonlySet<string> = new Set([
  ...
  // No native Windows app (name-substring only): Schwab, E*TRADE, TradeStation,
...
const POLICY_DENIED_BUNDLE_IDS: ReadonlySet<string> = new Set([
  ...
  // No native Windows app (name-substring only): Netflix, Disney+, Hulu,
...
const BROWSER_NAME_SUBSTRINGS: readonly string[] = [
  ...
  // bundle ID on Windows. The "... browser" entries below catch natural-language
...
const TERMINAL_NAME_SUBSTRINGS: readonly string[] = [
  // Windows / cross-platform terminals
  ...
  // (dev.warp.Warp-Stable, co.zeit.hyper) for Windows; Windows exe-name
  // matching can be added when Windows CU ships.
  // Windows shells (activate when the win32 gate lifts)
  "powershell",
  "cmd.exe",
  "command prompt",
  "git bash",
  ...
  // (false-positives). Covered by bundle ID (dev.zed.Zed) on Windows.
